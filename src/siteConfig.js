const siteConfig = {
  // Photo placed in /public (e.g., public/profile.jpg)
  // Replace "profile.jpg" with your actual filename once added to /public
  photo: "profile.jpg",
  initials: "HF",

  // Social links (edit with your real profiles)
  links: {
    facebook: "",
    linkedin: "https://www.linkedin.com/in/heike-christina-fenot-62828b1/"
  },

  // References shown in sidebar (below Coordonnées, above Liens)
  references: [
    {
      name: "Jean Dubois",
      company: "TEVEA RF Consulting",
      phone: "03.60.95.52.43",
      email: "jeandubois@jourrapide.com"
    },
    {
      name: "Sophie Lefèvre",
      company: "TEVEA RF Consulting",
      phone: "01.00.53.15.41",
      email: "sophielefevre@teleworm.us"
    }
  ],

  fr: {
    ui: {
      profile: "Profil",
      contact: "Coordonnées",
      competencies: "Compétences clés",
      experience: "Expérience",
      education: "Formations",
      languages: "Langues",
      interests: "Centres d’intérêt",
      website: "Site",
      linkedin: "LinkedIn",
      email: "Email",
      phone: "Tél.",
      nationality: "Nationalité",
      address: "Adresse",
      toggleLabel: "Langue",
      fr: "FR",
      en: "EN",
      other: "Autres expériences",
    },

    name: "Prénom NOM",
    role: "Directrice Pôle Chargés de Comptes",
    location: "Maisons‑Laffitte, France",
    email: "email@yahoo.fr",
    phone: "01 23 45 67 89",
    nationality: "Franco‑allemande",

    summary: [
      "15+ ans d’expérience en gestion de comptes clés, coordination de projets et amélioration des processus.",
      "Animation de rendez‑vous clients, formations et déploiements ; collecte et consolidation d’informations multi‑équipes.",
      "Développement commercial : propositions, négociations et croissance du portefeuille.",
    ],

    competencies: [
      "Gestion de comptes clés et relation client",
      "Pilotage de projets et coordination multi‑équipes",
      "Conception de processus et amélioration continue",
      "Animation de comités, ateliers et formations",
      "Rédaction de propositions commerciales",
    ],

    experience: [
      {
        title: "Directrice Pôle Chargés de Comptes - Client Relationship Department Director",
        company: "TEVEA RF Consulting",
        period: "Janv. 2022 — Aujourd’hui · Paris, Île-de-France (Hybride)",
        points: [
          "Direction du pôle Chargés de Comptes (management et performance).",
          "Pilotage de la relation client et de la satisfaction.",
          "Amélioration continue des processus et coordination transverse (TVA).",
          "Suivi d’indicateurs (KPI) et plan d’actions."
        ]
      },
      {
        title: "Gestionnaire de Comptes Senior — Senior Account Manager",
        company: "TEVEA RF Consulting",
        period: "Janv. 2017 — Déc. 2021 · Paris, Île-de-France",
        points: [
          "Gestion / Animation / Développement d’un portefeuille clients.",
          "Mise en œuvre d’un KYC Qualité et formalisation des besoins.",
          "Organisation et animation des RDV clients.",
          "Propositions marketing/commerciales et amélioration des processus.",
          "Commercialisation des services de l’offre TVA."
        ]
      },
      {
        title: "Gestionnaire Comptes Clients",
        company: "Directskills",
        period: "Oct. 2015 — Mai 2026",
        points: [
          "Management de grands comptes français : cadrage, suivi, satisfaction.",
          "Organisation et animation des rendez‑vous clients (comités, ateliers).",
          "Coordination et mise en place des processus de production côté client.",
          "Rédaction et maintien de la documentation/processus.",
          "Collecte, consolidation et transmission d’informations entre clients et équipes internes.",
          "Identification des besoins marché, remontée d’évolutions produit et services.",
          "Participation et lancement de nouveaux projets (déploiements, évolutions).",
          "Rédaction et défense de propositions commerciales ; développement du portefeuille.",
        ],
        about:
          "Directskills optimise le recours au Travail Temporaire (conseil achats, solution Cloud BAPS‑Intérim, services d’externalisation).",
      },
      {
        title: "Key Clients Relations Manager (Gestionnaire Grands Comptes)",
        company: "Lowendalmasaï",
        period: "Avr. 2008 — Oct. 2015",
        points: [
          "Gestion de comptes clés France/international, relation globale.",
          "Animation des RDV clients, création de supports et animation de formations.",
          "Coordination des processus de production et rédaction de procédures.",
          "Collecte, consolidation et transmission d’informations.",
          "Initialisation et participation à de nouveaux projets.",
          "Rédaction, validation et défense de propositions commerciales.",
          "Développement du portefeuille.",
        ],
        about:
          "Cabinet de conseil spécialisé en management des coûts, 425 personnes, présence dans 9 pays.",
      },
      {
        title: "Account Manager (Chargée d’affaires)",
        company: "Lowendalmasaï",
        period: "Sept. 2006 — Mars 2008",
        points: [
          "Prospection, négociation et signature de contrats (DG, DAF).",
          "Développement du portefeuille et gestion de la relation globale.",
        ],
      },
      {
        title: "Assistante Commerciale Aéronautique",
        company: "Trelleborg Sealing Solutions France",
        period: "Oct. 2005 — Juil. 2006",
        points: [
          "Gestion de la relation clients internationaux, prise de commandes.",
          "Mise en place de contrats, négociation achats‑ventes.",
        ],
      },
      {
        title: "Assistante Commerciale — Service Fiscal",
        company: "CCFA (CCI Française en Allemagne), Sarrebruck",
        period: "Janv. 2005 — Juin 2005",
        points: [
          "Prospection ciblée pour récupération de TVA étrangère.",
          "Création base clients, mailings, gestion relation clients DE/FR.",
        ],
      },
      {
        title: "Assistante Marketing et Commerciale",
        company: "Croso France",
        period: "2002 — 2004",
        points: [
          "Organisation d’événements et salons, dossiers de presse.",
          "Gestion placements publicitaires et mesure d’impact.",
          "Relation client et prise de commandes.",
        ],
      }
    ],

    otherExperience: [
      {
        title: "Stage en Ingénierie Financière",
        company: "Crédit Industriel et Commercial (CIC)",
        period: "Avr. 2002 — Mai 2002",
        points: [
          "Supports pour fusions‑acquisitions ; participation à réunions financières.",
        ],
      },
      {
        title: "Stage en Hôtellerie (Réception)",
        company: "IBIS Hôtel — Francfort (DE)",
        period: "Juil. 2001 — Août 2001",
        points: [
          "Accueil, gestion des départs, planning des réservations, questionnaires de satisfaction.",
        ],
      },
      {
        title: "Réceptionniste",
        company: "Regents Garden",
        period: "Juil. 2000 — Août 2000",
        points: ["Accueil clients, gestion réservations."],
      },
      {
        title: "Assistante Secrétariat",
        company: "B. Braun Médical",
        period: "Juil. 1999",
        points: ["Tâches administratives diverses."],
      },
    ],

    education: [
      {
        title:
          "École Supérieure de Commerce en Alternance (ISEE) — Paris (2002–2004)",
        detail: "Diplôme homologué niveau II.",
      },
      {
        title: "IUT de Sceaux (2000–2002)",
        detail:
          "DUT Techniques de Commercialisation, option Commerce International.",
      },
      {
        title:
          "Lycée Jeanne d’Albret — Saint‑Germain‑en‑Laye (2000)",
        detail: "Baccalauréat Littéraire.",
      },
    ],

    languages: ["Français (maternelle)", "Allemand (courant)", "Anglais (lu, écrit, parlé)"],
    interests: [
      "Natation (loisirs)",
      "Cinéma, expositions, théâtre",
      "Engagement associatif (parents d’élèves)",
    ],
  },

  en: {
    ui: {
      profile: "Profile",
      contact: "Contact",
      competencies: "Core Competencies",
      experience: "Experience",
      education: "Education",
      languages: "Languages",
      interests: "Interests",
      website: "Website",
      linkedin: "LinkedIn",
      email: "Email",
      phone: "Phone",
      nationality: "Nationality",
      address: "Address",
      toggleLabel: "Language",
      fr: "FR",
      en: "EN",
      other: "Other experiences",
    },

    name: "Prénom NOM",
    role: "Client Relationship Department Director",
    location: "Maisons‑Laffitte, France",
    email: "email@yahoo.fr",
    phone: "+33 1 23 45 67 89",
    nationality: "French‑German",

    summary: [
      "15+ years in key account management, project coordination and process improvement.",
      "Lead client meetings, trainings and deployments; collect and consolidate cross‑team information.",
      "Business development: proposals, negotiations and portfolio growth.",
    ],

    competencies: [
      "Key account management and client relationships",
      "Project governance and cross‑team coordination",
      "Process design and continuous improvement",
      "Client committees, workshops and training",
      "Commercial proposals and negotiations",
    ],

    experience: [
      {
        title: "Client Relationship Department Director",
        company: "TEVEA RF Consulting",
        period: "Jan 2022 — Present · Paris, Île-de-France (Hybrid)",
        points: [
          "Led the Client Relationship department (people leadership and performance).",
          "Owned client satisfaction and relationship governance.",
          "Drove continuous improvement across VAT-related processes.",
          "Monitored KPIs and delivered action plans."
        ]
      },
      {
        title: "Senior Account Manager",
        company: "TEVEA RF Consulting",
        period: "Jan 2017 — Dec 2021 · Paris Area, France",
        points: [
          "Managed, animated and grew a client portfolio.",
          "Implemented Quality KYC and captured client/prospect needs.",
          "Organized and led client meetings.",
          "Proposed marketing/commercial actions and improved processes.",
          "Sold the full VAT services offering."
        ]
      },
      {
        title: "Key Account Manager",
        company: "Directskills",
        period: "Oct 2015 — May 2026",
        points: [
          "Own large French accounts: scoping, follow‑up and client satisfaction.",
          "Organize and run client meetings (steering, workshops).",
          "Coordinate and implement client production processes.",
          "Write and maintain process documentation.",
          "Collect, consolidate and route information between client and internal teams.",
          "Identify market needs and relay product/service enhancements.",
          "Initiate and support new projects (deployments, evolutions).",
          "Draft and defend commercial proposals; grow the portfolio.",
        ],
        about:
          "Directskills optimizes the use of temporary work in France (procurement consulting, Cloud solution BAPS‑Intérim, and outsourcing services).",
      },
      {
        title: "Key Clients Relations Manager",
        company: "Lowendalmasaï",
        period: "Apr 2008 — Oct 2015",
        points: [
          "Manage key accounts in France and abroad, end‑to‑end relationship.",
          "Lead client meetings; create materials and deliver trainings.",
          "Coordinate production processes and write procedures.",
          "Collect, consolidate and share information across teams.",
          "Launch and participate in new projects.",
          "Draft, validate and defend proposals; develop the portfolio.",
        ],
        about:
          "Consulting firm specialized in cost management; 425 staff; presence in 9 countries.",
      },
      {
        title: "Account Manager",
        company: "Lowendalmasaï",
        period: "Sep 2006 — Mar 2008",
        points: [
          "Prospecting, negotiation and contract signing (CEO, CFO).",
          "Portfolio development and overall client relationship.",
        ],
      },
      {
        title: "Sales Assistant — Aerospace",
        company: "Trelleborg Sealing Solutions France",
        period: "Oct 2005 — Jul 2006",
        points: [
          "Global relationship with international clients; order intake.",
          "Contract setup; purchasing and sales negotiations.",
        ],
      },
      {
        title: "Sales Assistant — Tax Department",
        company: "French CCI in Germany (CCFA), Saarbrücken",
        period: "Jan 2005 — Jun 2005",
        points: [
          "Targeted prospecting for foreign VAT recovery.",
          "Built customer database, mailings; managed FR/DE client relations.",
        ],
      },
      {
        title: "Marketing & Sales Assistant",
        company: "Croso France",
        period: "2002 — 2004",
        points: [
          "Organized events and trade fairs; press kits.",
          "Managed ad placements and measured impact.",
          "Client relationship and order intake.",
        ],
      }
    ],

    otherExperience: [
      {
        title: "Intern — Financial Engineering",
        company: "CIC (Crédit Industriel et Commercial)",
        period: "Apr 2002 — May 2002",
        points: [
          "Prepared materials for M&A; attended financial meetings.",
        ],
      },
      {
        title: "Intern — Hospitality (Front Desk)",
        company: "IBIS Hotel — Frankfurt (DE)",
        period: "Jul 2001 — Aug 2001",
        points: [
          "Welcomed guests, managed stays and departures; reservations planning; satisfaction surveys.",
        ],
      },
      {
        title: "Receptionist",
        company: "Regents Garden",
        period: "Jul 2000 — Aug 2000",
        points: ["Front desk operations; reservations planning."],
      },
      {
        title: "Administrative Assistant",
        company: "B. Braun Medical",
        period: "Jul 1999",
        points: ["Various administrative duties."],
      },
    ],

    education: [
      {
        title:
          "Business School (ISEE) — Paris (2002–2004)",
        detail: "Diploma recognized at Level II (FR).",
      },
      {
        title: "IUT de Sceaux (2000–2002)",
        detail:
          "University Diploma in Sales Techniques (DUT), International Trade option.",
      },
      {
        title:
          "Lycée Jeanne d’Albret — Saint‑Germain‑en‑Laye (2000)",
        detail: "Baccalauréat (Literature).",
      },
    ],

    languages: ["French (native)", "German (fluent)", "English (read, written, spoken)"],
    interests: [
      "Swimming (leisure)",
      "Cinema, exhibitions, theatre",
      "Parent association volunteer",
    ],
  },
};

export default siteConfig;
